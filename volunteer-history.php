...
while ($row = mysqli_fetch_row($results)) {
    ...
    echo '<h3>' . $event[2] . '</h3>';
    echo '<p>' . $event[12] . '</p>';
    $queryOrg = "SELECT name FROM organisations WHERE id = '$event[1]'";
    $orgResults = mysqli_query($link,$queryOrg);
    $org = mysqli_fetch_row($orgResults);
    echo '<h5>Posted by: ' . $org[0] . '</h5>';
    echo '<h5>Category: ' . $event[3] . '</h5>';
    echo '<h5>Date: ' . $event[8] . '</h5>';
    echo '<h5>Area: ' . $event[7] . '</h5>';
    echo '<a href="event.php?id=' . $event[0] . '">Read more &raquo;</a>';
    echo '</div>';
}
